...
import {getApiData} from "../api/twitter/getApiData";
...
export default function TwitterAnalytics() {
  ...
  const handleTwitterCallback = async () => {
    ...
    try {
      if (code) {
        ...
        if (response.ok) {
          ...
          setUserData(responseData);
            // Call getApiData with the access token
            const apiDataResponse = await getApiData(responseData.access_token);
            console.log("API Data: ", apiDataResponse);
            // Set the API data in state
            setApiContent(apiDataResponse);
      ...
      // Handle fetch errors or any other unexpected errors
      console.error('Error in Twitter callback: ', error);
    }
  };
  useEffect(() => {
  // Call handleTwitterCallback when the code parameter is available
  ...
  return (
    ...
    {userData && (
      <div>
        ...
        <p>ApiContent: {JSON.stringify(apiContent)}</p>
      </div>
    )}
  </div>
  );
